...
## キーマップの変更
Textやオブジェクト図において、インラインでテキストを入力するとき、IMEを使わずに直接入力すると、正しい記号を入力できないことがありました。2026/04/25のバージョンでは記号を置き換える改造をしています。（ただし、`|`は入力できなくなりました）。

変換させたくなければ、`dia.exe` のファイル名を `dia101.exe` に変更してください。


### 原因と改造

キー入力のイベントハンドラが定義されている `app/disp_callback.c` において、描画オブジェクトの `handle_key_event()` に渡す文字列として、deprecated な GdkEventKeyのメンバstringが使われているためのようです。

`lib/dia-text.c` の `handle_key_event()` において、ルックアップテーブルを使って変換しています。
